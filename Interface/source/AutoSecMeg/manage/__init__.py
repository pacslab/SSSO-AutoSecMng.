...
import sys
sys.path.append("..")
from AutoSecMeg.sssontology import sssontology
import AutoSecMeg.exceptions
from AutoSecMeg.utils import endpoint_request
...
class Equipment(object):
    ...
    def create(self, equipmentModel, context=None):
        ...
        if type(equipmentModel) == str:
            ...
            if equipmentModel == []:
                raise AutoSecMeg.exceptions.URIDoesNotExist
            else:
            ...
            if self.ontology.exists(EQM_UUID):
                raise AutoSecMeg.exceptions.URIAlreadyExists(EQM_UUID, 'Equipment Model')
        ...
        for item in equipmentModel.get('Service'):
            ...
            if serviceSecurityAssessmentPolicy is not None:
                if not self.ontology.exists(serviceSecurityAssessmentPolicy):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(serviceSecurityAssessmentPolicy, 'Security_Assessment_Policy')
            ...
            if serviceAccessPolicy is not None:
                if not self.ontology.exists(serviceAccessPolicy):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(serviceAccessPolicy, 'Access_Policy')
            ...
            if serviceTrustLevelAssessmentPolicy is not None:
                if not self.ontology.exists(serviceTrustLevelAssessmentPolicy):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(serviceTrustLevelAssessmentPolicy, 'Trust_Level_Assessment_Policy')
            ...
            if serviceMetadata is not None:
                if not self.ontology.exists(serviceMetadata):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(serviceMetadata,'Request Model')
        ...
        for item in equipmentModel.get('threat'):
            ...
            if threatAssessmentPolicy is not None:
                if not self.ontology.exists(threatAssessmentPolicy):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(threatAssessmentPolicy, 'Threat_Assessment_Policy')
            ...
            if threatMitigationPolicy is not None:
                if not self.ontology.exists(threatMitigationPolicy):
                    raise AutoSecMeg.exceptions.URIDoesNotExist(threatMitigationPolicy, 'Threat_Mitigation_Policy')
    ...
    def enable(self, serviceUUID, userUUID):
        ...
        if serviceTopClass == 'Authentication':
            ...
            if SLTLAPClass == 'Trust_Level':
                ...
                New_TrustLevel = min([4, int(Provided_TrustLevel[-1])+User_TrustLevel])
                ...
                New_TrustLevel = min([4, int(evaluation_res[-1])+User_TrustLevel])
                New_TrustLevel = f'SL-{New_TrustLevel}'
                self.ontology.update_user_trust_level(userUUID, New_TrustLevel)
                Previous_Authentication_Methods.append(Current_Authentication_Method)
                User_Data['Authentication'] = Previous_Authentication_Methods
        ...
        if SLClass == 'Security_Level':
            ...
            if(User_TrustLevel>=Env_SecurityLevel):
                ...
                return (False, 'Trust_Level', max([Env_SecurityLevel-User_TrustLevel, Required_SecurityLevel-User_TrustLevel]))
...
class Policy(object):
    ...
    def refresh_data(self, policy, userUUID, serviceUUID):
        ...
        if endpoint.get('Type')=='SPARQL':
            ...
            if endpoint_list is None:
                raise AutoSecMeg.exceptions.EndpointInaccessible(endpoint_list)
    ...
    def add_policy(self, policyDict, policyClass):
        ...
        if self.ontology.exists(policyUUID):
            raise AutoSecMeg.exceptions.URIAlreadyExists(policyUUID, policyClass)
        bulk_updates = []
        bulk_updates += [
    ...
    def remove_policy(self, policyUUID):
        if self.ontology.exists(policyUUID):
            raise AutoSecMeg.exceptions.URIDoesNotExist(policyUUID)
...
class Context(object):
    ...
    def refresh_endpoint_data(self, endpoint_q, userUUID, serviceUUID):
        ...
        try:
            ...
            raise AutoSecMeg.exceptions.EndpointInaccessible
    ...
    def add_context(self, contextIndividual, contextClass, dataproperty=None, data=None, datatype='Literal'):
        ...
        if self.ontology.exists(contextIndividual):
            raise AutoSecMeg.exceptions.URIAlreadyExists(contextIndividual)
        bulk_updates = []
        bulk_updates += [
...
class User(object):
    ...
    def add_user(self, user, context):
        if self.ontology.exists(user):
            raise AutoSecMeg.exceptions.URIAlreadyExists(user)
        bulk_updates = []
        bulk_updates += [
...
class Manager(object):
    def __init__(self):
        ...
        ttl_filename = None
        jena_address = os.getenv('JENA_ADDRESS', None)
        if jena_address is None:
            ttl_filename = os.path.join(BASE_DIR, 'ssso.ttl')
